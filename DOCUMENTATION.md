...
Core business logic lives in the services (`service/`), which talk to the database through Exposed (`db/`) and provide game operations (fishing, regular tournaments, special club events, shop, referrals). The database layer uses Hikari for JDBC pooling and exposes a bounded dispatcher helper for blocking Exposed work from Ktor routes. Routes in `app/ApiRoutes.kt` rely on these services and serializers to build responses for the Mini App, PlayDeck wrapper, Android client, or bot. Protected operator routes live in `app/AdminApiRoutes.kt`. The auth/session layer now supports Telegram cookie sessions, PlayDeck wrapper initData fallback, and bearer tokens backed by refresh sessions.
