(() => {
  ...
  const STRINGS = {
    ru: {
      ...
      shopDiscountInfo: ({ percent, until }) => `скидка ${percent}% от полной цены${until ? ` до ${until}` : ''}`,
    ...
    en: {
      ...
      shopDiscountInfo: ({ percent, until }) => `discount ${percent}% off full price${until ? ` until ${until}` : ''}`,
