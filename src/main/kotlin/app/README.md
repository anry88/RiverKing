...
- **`ApiRoutes.kt`** — `Application.apiRoutes(env)` registers all REST endpoints for the mini‑app. Notable groups:
  ...
  - Authentication: `POST /api/auth/telegram` accepts `initData`, verifies the signature with `TgWebAppAuth.verifyAndExtractUser`, falls back to PlayDeck `initdata/check` for wrapper-signed payloads, and stores `AppSession(userId)`. Mobile auth is handled by `POST /api/auth/google`, `POST /api/auth/password/register`, `POST /api/auth/password/login`, `POST /api/auth/refresh`, and `POST /api/auth/logout`.
  ...
  - PlayDeck payments: `POST /api/playdeck/order` creates signed external order IDs for wrapper `requestPayment`; `POST /api/playdeck/postback` validates PlayDeck HMAC postbacks for checkout/payment events before granting packages and recording idempotent payments.
...
- **`BotRoutes.kt`** — registers the Telegram bot webhook. Processes updates, sends responses through `TelegramBot`, and calls game services to handle bot commands, including the three-section `/quests` output with club quests, `/club` membership and club-management flows, `/event` current special-event standings, `/start` launch links, and `/location` menus in unlock order.
...
- **`PlayDeckAuth.kt`** — validates PlayDeck wrapper `tgWebAppData` through PlayDeck's initData check endpoint when the standard bot-token Telegram verification does not apply.
