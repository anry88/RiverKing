...
object DB {
    ...
    private fun seedIfEmpty() {
        ...
        // --- Fish (как у тебя + добавления для баланса) ---

        // Пресные базовые/продвинутые
        ...
        val fGly = upsertFish("Гольян",      "common",   0.02,0.01,false,"fresh") // НОВОЕ: мирная для горных ручьев

        // Морские/солоноватые (прибрежье/шельф/фьорды)
        ...
        val fAnc = upsertFish("Анчоус",   "common", 0.02,0.01,false,"salt")   // НОВОЕ

        // Открытый океан — пелагика
        ...
        // простые
        setLFWeight(pond, fPe, 0.7); setLFWeight(pond, fEr, 0.6); setLFWeight(pond, fGu2,0.5)
        setLFWeight(pond, fKr2,0.7); setLFWeight(pond, fVh, 0.5)
        ...
        // простые
        setLFWeight(river, fPe,0.6); setLFWeight(river, fEr,0.5); setLFWeight(river, fEl2,0.4)
        setLFWeight(river, fKr2,0.5); setLFWeight(river, fYa2,0.25)
        ...
        // простые
        setLFWeight(lake, fGu2,0.6); setLFWeight(lake, fKr2,0.6); setLFWeight(lake, fEr,0.4)
        setLFWeight(lake, fPe,0.5); setLFWeight(lake, fEl2,0.3); setLFWeight(lake, fYa2,0.2)
        ...
        // простые
        setLFWeight(swamp, fGu2,0.7); setLFWeight(swamp, fKr2,0.8); setLFWeight(swamp, fEr,0.4)
        setLFWeight(swamp, fPe,0.4); setLFWeight(swamp, fVh,0.5)

        // Горная река — ДОБАВЛЕНЫ мирные цели (пресная мирная приманка)
        ...
        // мирные для горной воды:
        setLFWeight(mtnRiver, fEl2,0.35) // елец
        setLFWeight(mtnRiver, fPe, 0.35) // пескарь
        setLFWeight(mtnRiver, fGly,0.45) // гольян (новая опора под пресную мирную)
        // чуть «погран.»
        setLFWeight(mtnRiver, fSi, 0.10) // сиг местами
        ...
        // Дельта — смешанная солоноватая зона (расширили морских мирных)
        ...
        setLFWeight(delta, fSar,0.25); setLFWeight(delta, fAnc,0.20) // НОВОЕ
        // salt хищные/эпики:
        ...
        // Прибрежье моря — добавили морских мирных (чтобы «морская мирная» работала часто)
        setLFWeight(coast, fMu,0.9); setLFWeight(coast, fHe,0.9)   // кефаль/сельдь
        setLFWeight(coast, fFl,0.6); setLFWeight(coast, fSt,0.6); setLFWeight(coast, fSe,0.3)
        setLFWeight(coast, fSar,0.6); setLFWeight(coast, fAnc,0.5) // НОВОЕ: сардина/анчоус
        ...
        // редкие залёты пелагики у берега
        setLFWeight(coast, fDor,0.03); setLFWeight(coast, fWah,0.02); setLFWeight(coast, fSail,0.01)
        // Белуга
        setLFWeight(coast, fBel,0.01)

        // Фьорд — холодные воды, но морские мирные тоже бывают
        setLFWeight(fjord, fCo,0.9); setLFWeight(fjord, fSa,0.7); setLFWeight(fjord, fHe,0.6); setLFWeight(fjord, fSe,0.4)
        setLFWeight(fjord, fHa2,0.08); setLFWeight(fjord, fSal,0.10)
        setLFWeight(fjord, fSm,0.12)   // корюшка
        // добавить мирных: сардина (умеренно), анчоус (реже)
        setLFWeight(fjord, fSar,0.12); setLFWeight(fjord, fAnc,0.08)
        // мелочь
        ...
        setLFWeight(openOcean, fBmr,0.05); setLFWeight(openOcean, fTbf,0.04)
        // немного морских мирных косяков далеко от берега:
        setLFWeight(openOcean, fSar,0.15); setLFWeight(openOcean, fAnc,0.12)
        setLFWeight(openOcean, fSar,0.15); setLFWeight(openOcean, fAnc,0.12)

        // --- Приманки (без изменений по названиям/бонусам) ---
