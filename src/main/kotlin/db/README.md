...
- **`Tables.kt`** — contains table definitions (`Users`, `Lures`, `Rods`, `Locations`, `Catches`, `Tournaments`, `Prizes`, `Payments`, `Achievements`, `QuestProgress`, `Clubs`, `ClubMembers`, `ClubWeeklyContributions`, `ClubWeeklyRewards`, etc.). It also stores helper mappings for relations between users, prizes, and tournaments.
