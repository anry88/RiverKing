...
- **`Tables.kt`** — contains table definitions (`Users`, `AuthIdentities`, `PasswordCredentials`, `AuthSessions`, `Lures`, `Rods`, `Locations`, `Catches`, `Tournaments`, `Prizes`, `Payments`, `Achievements`, `QuestProgress`, `Clubs`, `ClubMembers`, `ClubWeeklyContributions`, `ClubWeeklyRewards`, etc.). `Users.tg_id` is now a nullable legacy link for Telegram, while auth providers live in the dedicated auth tables.
